...
- `src/admin/` — the Admin Panel (`/admin/*`, see `CONTEXT.md` and [ADR 0005](./docs/adr/0005-admin-panel-in-web-client.md)). `App.tsx` imports `AdminRoutes` via `React.lazy`/`Suspense` specifically so none of this — including the `@mdxeditor/editor` dependency — ships in the public page's bundle; don't import anything from `src/admin/` outside that lazy boundary. `AuthContext` holds the JWT in `localStorage` (see [ADR 0006](./docs/adr/0006-admin-session-model.md)) and calls `GET /auth/me` on mount to validate it, clearing it on failure. `pages/PostEditorPage.tsx`'s MDXEditor needs `className="dark-theme"` to pick up dark styling and `contentEditableClassName="prose prose-invert ..."` (from `@tailwindcss/typography`, registered via `@plugin` in `index.css`) for heading/list sizing — Tailwind's preflight otherwise strips heading styles entirely. Its `imagePlugin({ imageUploadHandler })` posts to `POST /uploads` and inserts the returned URL.
